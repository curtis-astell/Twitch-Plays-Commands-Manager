...
class Twitch:
    def __init__(self, append_text_func):
        self.append_text = append_text_func
    re_prog = None
    sock = None
    partial = b''
    login_ok = False
    ...
    def twitch_connect(self, channel):
        countdown = 5
        while countdown > 0:
            self.append_text(str(countdown))
            countdown -= 1
            time.sleep(1)
        ...
        # Create socket
        self.append_text('Connecting to Twitch...')
        ...
        self.append_text('Connected to Twitch. Logging in anonymously...')
    ...
    def receive_and_parse_data(self):
        ...
        while True:
            ...
            #         break
                ...
                self.append_text('Unexpected connection error. Reconnecting in a second...', e)
                self.reconnect(1)
                return []
            if not received:
                self.append_text('Connection closed by Twitch. Reconnecting in 5 seconds...')
                self.reconnect(5)
                return []
            buffer += received
        ...
        if buffer:
            ...
            if not matches:
                ...
                if matches[0].start() != 0:
                    ...
                    self.append_text('either ddarknut fucked up or twitch is bonkers, or both I mean who really knows anything at this point')
    ...
    def twitch_receive_messages(self):
        ...
        for irc_message in self.receive_and_parse_data():
            ...
            if cmd == 'PRIVMSG':
                ...
                self.append_text('Successfully logged in. Joining channel %s.' % self.channel)
                self.sock.send(('JOIN #%s\r\n' % self.channel).encode())
                self.login_ok = True
            elif cmd == 'JOIN':
                self.append_text('Successfully joined channel %s' % irc_message['params'][0])
            elif cmd == 'NOTICE':
                self.append_text('Server notice:', irc_message['params'], irc_message['trailing'])
                ...
                self.append_text('Unhandled irc message:', irc_message)
        ...
        if not self.login_ok:
            ...
            if time.time() - self.login_timestamp > MAX_TIME_TO_WAIT_FOR_LOGIN:
                self.append_text('No response from Twitch. Reconnecting...')
                self.reconnect(0)
                return []
...
class YouTube:
    ...
    def reconnect(self, delay):
        if self.fetch_job and self.fetch_job.running():
            if not fetch_job.cancel():
                self.append_text("Waiting for fetch job to finish...")
                self.fetch_job.result()
        self.append_text(f"Retrying in {delay}...")
    ...
    def youtube_connect(self, channel_id, stream_url):
        self.append_text("Connecting to YouTube...")
        ...
        if not res.ok:
            if stream_url is not None:
                self.append_text(f"Couldn't load the stream URL ({res.status_code} {res.reason}). Is the stream URL correct? {self.stream_url}")
            else:
                self.append_text(f"Couldn't load livestream page ({res.status_code} {res.reason}). Is the channel ID correct? {self.channel_id}")
            time.sleep(5)
            exit(1)
        ...
        if len(matches) == 0:
            self.append_text("Couldn't find initial data in livestream page")
            time.sleep(5)
            exit(1)
        ...
        try:
            ...
            self.append_text(f"Couldn't find the livestream chat. Is the channel not live? url: {live_url}")
            time.sleep(5)
            exit(1)
        ...
        if not res.ok:
            self.append_text(f"Couldn't load live chat page ({res.status_code} {res.reason})")
            time.sleep(5)
            exit(1)
        ...
        if len(matches) == 0:
            self.append_text("Couldn't find initial data in live chat page")
            time.sleep(5)
            exit(1)
        initial_data = json.loads(matches[0].group(1))
        ...
        if len(matches) == 0:
            self.append_text("Couldn't find config data in live chat page")
            time.sleep(5)
            exit(1)
    ...
    def fetch_messages(self):
        ...
        if not res.ok:
            self.append_text(f"Failed to fetch messages. {res.status_code} {res.reason}")
            self.append_text("Body:", res.text)
            self.append_text("Payload:", payload_bytes)
            self.session.close()
            self.session = None
            return []
        try:
            ...
            self.append_text(f"Failed to parse messages.")
            self.append_text("Body:", res.text)
            traceback.print_exc()
        return []
